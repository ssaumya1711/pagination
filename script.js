...
localStorage.setItem("lastindex",n);
function display(a){
  document.querySelector(".table").remove();
     const table = document.createElement('table');
     table.className = "table";
     table.innerHTML = `<tr><th>Name</th><th>Email</th></tr>`;
  for(let j = (a-1)*10;j<(a*10);j++)
     {
       const row = document.createElement('tr');
       const name = document.createElement('td');
       name.innerText = obj[j].name;
       const email = document.createElement('td');
       email.innerText = obj[j].email;
...
function prevpage(){
  let x = localStorage.getItem("index") - 1;
  if(x == 0)
    x = 1;
...
pagination.innerHTML = '<button class="first" onclick="firstpage()">First</button><button class="prev" onclick="prevpage()">Prev</button>';
for(let i = 1;i<=n;i++)
{
   ...
   page.onclick = function(){
     ...
     display(localStorage.getItem("index"));
   }
...
next.onclick = function(){
  let y = parseInt(localStorage.getItem("index")) + 1;
  if(y > localStorage.getItem("lastindex"))
    y = localStorage.getItem("lastindex");
